...
const gridMap = input.split("\n").map((row) => row.split(""));

const sPosition = gridMap
  .map((row, rowIndex) =>
    row.map((cell, colIndex) => [cell, rowIndex, colIndex])
  )
  .flat()
  .filter(([cell]) => cell === "S")[0]
  .slice(1)
  .map((x) => Number(x));

const possibleSymbols = ["-", "|", "J", "7", "L", "F", "S"];

function detectNeighbors(pos: number[], symbol: string = null) {
  const [x, y] = pos;
  ...
  let validNeighbors: number[][] = [];

  switch (symbol || gridMap[y][x]) {
    case "-":
      validNeighbors = [neighbors[0], neighbors[2]]; // left and right
      break;
    case "|":
      validNeighbors = [neighbors[1], neighbors[3]]; // up and down
      break;
    case "J":
      validNeighbors = [neighbors[0], neighbors[1]]; // left and up
      break;
    case "7":
      validNeighbors = [neighbors[0], neighbors[3]]; // left and down
      break;
    case "L":
      validNeighbors = [neighbors[1], neighbors[2]]; // up and right
      break;
    case "F":
      validNeighbors = [neighbors[3], neighbors[2]]; // right and down
      break;
  }

  return validNeighbors
    .filter((e) => gridMap[e[1]]?.[e[0]] !== undefined)
    .filter(([x, y]) => possibleSymbols.includes(gridMap[y][x]));
}

let beforePositions = new Map<string, string>();

function startFromPositionWithSymbol(pos: number[], symbol: string = null) {
  const [y, x] = pos;

  const neighbors = detectNeighbors(pos, symbol);

  beforePositions.set(pos.join(","), `${symbol || gridMap[y][x]}`);

  neighbors.forEach((neighbor) => {
    if (!beforePositions.has(neighbor.join(","))) {
      startFromPositionWithSymbol(neighbor);
    }
  });
...
const pos = startFromPositionWithSymbol(sPosition, "F");
console.log(beforePositions);
console.log(pos.size / 2);
