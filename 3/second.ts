...
const input = `................................................965..583........389.................307.................512......................395.....387
...
function detectNumberInitialPosition(i: number, j: number) {
  ...
  while (nb.includes(splittedInput[i][y])) {
    y--;
  }
...
function getNumberFromPosition(i: number, j: number) {
  let y = j;
  let number = "";

  while (nb.includes(splittedInput[i][y])) {
    number += splittedInput[i][y];
    y++;
  }
...
function returnPositionOfAdjacentNumbersFromSymbol(i: number, j: number) {
  const adjacentNumbers = new Set<string>();

  // Detect all adjacent numbers of a symbol
  // and return a set of their positions.
  // Use detectNumberInitialPosition() to get the
  // initial position of each number.

  for (let x = i - 1; x <= i + 1; x++) {
    for (let y = j - 1; y <= j + 1; y++) {
      if (nb.includes(splittedInput[x]?.[y])) {
        const newY = detectNumberInitialPosition(x, y);
        adjacentNumbers.add(`${x}, ${newY}`);
      }
    }
  }
...
for (let i = 0; i < splittedInput.length; i++) {
  for (let j = 0; j < splittedInput[i].length; j++) {
    if (!notSymbols.includes(splittedInput[i][j])) {
      const adjacentNumbersPos = returnPositionOfAdjacentNumbersFromSymbol(
        i,
        j
      );

      if (adjacentNumbersPos.size === 2) {
        const adjacentNumbers = Array.from(adjacentNumbersPos).map((pos) => {
          const [x, y] = pos.split(", ");
          return getNumberFromPosition(+x, +y);
        });
