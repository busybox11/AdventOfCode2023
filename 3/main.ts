...
const nbPlacements = new Set<string>();

for (let i = 0; i < splittedInput.length; i++) {
  for (let j = 0; j < splittedInput[i].length; j++) {
    if (splittedInput[i][j] !== ".") {
      if (detectNumberHasAdjacentSymbol(i, j)) {
        const y = detectNumberInitialPosition(i, j);
        nbPlacements.add(`${i}, ${y}`);
      }
    }
  }
}
...
for (const coord of nbPlacements) {
  const [i, j] = coord.split(", ").map(Number);
  const number = getNumberFromPosition(i, j);
