...
// This function will recognize all numbers in a string
// and return an array containing all of them in order.
// Numbers can either be an int, or fully spelled out.
// For example, "one" and "1" will both be recognized.
// However, there are no spaces allowed in the string.
// You must start to recognize a number for every character.
// Return all numbers in the order they appear in the string,
// as ints.
...
function recognize(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    while (j < str.length) {
      num += str[j];
      if (nbrs.includes(num)) {
        // Use the index of the number in the array
        // If it's more than 10, it's the index - 10

        let index = nbrs.indexOf(num);
        if (index > 10) {
          index -= 10;
        }
        result.push(index);
...
for (let line of str.split("\n")) {
  let value: string = recognize(line);
  ...
  if (value.length === 1) {
    nb = parseInt(`${value}${value}`);
  }

  if (value.length >= 2) {
    nb = parseInt(`${value[0]}${value[value.length - 1]}`);
  }
...
// Sum of all numbers
console.log(outValues.reduce((a, b) => a + b, 0));
