...
function parseLine(line) {
  ...
  const gameID = Number(name.replace("Game ", ""));

  const games = set.split(";").map((game) => {
    const colors = game.split(",").map((color) => {
      const [amount, colorName] = color.trim().split(" ");

      return { amount: Number(amount), color: colorName };
    });
...
for (const gameObj of games) {
  ...
  for (const game of games) {
    for (const { amount, color } of game) {
      if (amount > minColors[color]) {
        minColors[color] = amount;
      }
    }
  }

  powers.push(minColors.red * minColors.green * minColors.blue);
}

// Return sum of valid game IDs
